...
const Sidebar = ({ conversations, onSelect, activeId, onContextMenu, onTogglePin, onDelete, onUpdateTag }) => {
  ...
  const getPreviewText = (conv) => {
    ...
    if (raw.startsWith('{') && raw.includes('callType')) {
      try {
        ...
        return data.callType === 'video' ? t('sidebar.video_call') : t('sidebar.voice_call');
      } catch (e) {
  ...
  const formatLastSeen = (status, lastSeenAt) => {
    ...
    if (diff < 60) return t('sidebar.just_now');
    if (diff < 3600) {
      const mins = Math.floor(diff / 60);
      return t('sidebar.minutes_ago', { count: mins });
    }
    if (diff < 86400) {
      const hours = Math.floor(diff / 3600);
      return t('sidebar.hours_ago', { count: hours });
    }
    const days = Math.floor(diff / 86400);
    return t('sidebar.days_ago', { count: days });
  };
    ...
    <div className="flex flex-col h-full bg-sidebar transition-colors relative">
      {sortedConversations.length === 0 ? (
        <div className="p-12 text-center space-y-2">
          <p className="text-[10px] font-mono font-black uppercase tracking-[0.3em] text-foreground/40">{t('sidebar.no_conversations')}</p>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-0.5">
                  <h3 className={`text-[15px] tracking-tight truncate ${isActive ? 'text-indigo-700 dark:text-indigo-300' : 'text-foreground'} ${conv.unreadCount > 0 ? 'font-black' : 'font-bold'}`}>
                    {conv.type === 'SINGLE' 
                      ? (otherMember?.fullName || otherMember?.name || conv.name || t('sidebar.user_fallback'))
                      : (conv.name || t('sidebar.group_fallback'))}
                  </h3>
                  ...
                  <div className={`text-[13px] truncate leading-tight flex-1 pr-2 flex items-center ${conv.unreadCount > 0 ? 'text-indigo-500 dark:text-indigo-400 font-bold' : 'text-foreground/60 font-medium'}`}>
                    ...
                    <div className="relative mr-2 flex-shrink-0">
                      <button
                        ...
                        title={t('sidebar.tag_title_tooltip')}
                      />
                          ...
                          <div
                           ...
                           >
                             <div className="px-3 py-1 mb-1 border-b border-border/40">
                               <p className="text-[9px] font-bold uppercase text-foreground/40 tracking-widest">{t('sidebar.tags_title')}</p>
                             </div>
                             {TAGS.map(tag => (
                               <button
                                 key={tag.key}
                             ...
                             {conv.tag && (
                               ...
                               >
                                 ...
                                 <span className="text-[12px] font-semibold text-red-500">{t('sidebar.remove_tag')}</span>
                               </button>
                             )}
                          </div>
                        </>
                      ...
                      {conv.lastMessage ? (
                        <>
                          {conv.lastMessageSenderId === (user?.userId || user?.id) ? (
                            <span className="text-indigo-500 dark:text-indigo-400/70 mr-1 font-bold">{t('sidebar.you')}</span>
                        ...
                        <span className="italic text-foreground/30 text-[11px] font-bold">{statusText || t('sidebar.start_conversation')}</span>
                      )}
                    </div>
                  </div>
